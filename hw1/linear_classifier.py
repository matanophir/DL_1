...
class LinearClassifier(object):
    ...
    def train(
        ...
        for epoch_idx in range(max_epochs):
            ...
            for x, y in dl_train:
                ...
                loss += 1/2 * weight_decay * (self.weights**2).sum()

                grad = loss_fn.grad()
                self.weights -= learn_rate * grad

                losses.append(loss.item())
                accuracy.append(self.evaluate_accuracy(y, y_pred).item())
            
            train_res.loss.append(sum(losses) / len(losses))
            train_res.accuracy.append(sum(accuracy) / len(accuracy))
            ...
            valid_res.loss.append(sum(losses) / len(losses))
            valid_res.accuracy.append(sum(accuracy) / len(accuracy))
            # ========================
            print(".", end="")
    ...
    def weights_as_images(self, img_shape, has_bias=True):
        ...
        w_images = weights.T.reshape(self.n_classes, *img_shape)  # like ravel and order.
        # ========================

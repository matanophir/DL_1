...
class LinearRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        # ====== YOUR CODE: ======
        N = X.shape[0]
        reg_matrix = self.reg_lambda * np.eye(X.shape[1])
        reg_matrix[0, 0] = 0 # exclude bias term from regularization
        w_opt = np.linalg.inv((X.T @ X)/N + reg_matrix) @ (X.T @ y)/N
        # ========================
...
def fit_predict_dataframe(
    ...
    # ====== YOUR CODE: ======
    feature_names = df.columns.drop(target_name) if feature_names is None else feature_names
...
class BostonFeaturesTransformer(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, degree=2):
        ...
        # ====== YOUR CODE: ======
        # raise NotImplementedError() # what hyperparameters do I need?
        # ========================
    ...
    def transform(self, X):
        ...
        np.delete(X_transformed, 3, axis=1) # remove 'CHAS' column
        X_transformed[:, 0] = np.log(X_transformed[:, 0]) # 'CRIM'
        X_transformed[:, 12] = np.log(X_transformed[:, 12]) # 'LSTAT'
...
def top_correlated_features(df: DataFrame, target_feature, n=5):
    ...
    # ====== YOUR CODE: ======
    top_n = df.loc[:, df.columns != target_feature ].corrwith(df[target_feature]).sort_values(key=abs, ascending=False)[:n]
    top_n_features = top_n.index
    top_n_corr = top_n.values
    # ========================
...
def mse_score(y: np.ndarray, y_pred: np.ndarray):
    ...
    # ====== YOUR CODE: ======
    N = y.size
    mse = 1/N * np.sum((y - y_pred) ** 2) #TODO why not 1/2N as written? 
    # ========================
    return mse
...
def cv_best_hyperparams(
    ...
    for params in all_params:
        model.set_params(bostonfeaturestransformer__degree=params[0], linearregressor__reg_lambda=params[1])
        scores = np.array([])

        for train_index, val_index in sklearn.model_selection.KFold(n_splits=k_folds).split(X):
            X_train, X_val = X[train_index], X[val_index]
            y_train, y_val = y[train_index], y[val_index]
            model.fit(X_train, y_train)
            y_pred = model.predict(X_val)
            scores = np.append(scores, mse_score(y_val, y_pred))
